...
function prettyPrint(argv, object, rules) {
  const host = argv.host === '0.0.0.0' ? 'death_star' : argv.host
...
module.exports = function(argv) {
  ...
  function start(cb) {
    ...
    console.log(chalk.gray('  Building', source))
    ...
    load(source, (err, data) => {
      ...
      let routes
      if (argv.routes) {
        console.log(chalk.gray('  Building', argv.routes))
      ...
      if (argv.middlewares) {
        middlewares = argv.middlewares.map(function(m) {
          console.log(chalk.gray('  Building', m))
          return require(path.resolve(m))
        })
      }

      // Done
      console.log(chalk.gray('  The Death Star is ready to serve the Empire!'))
